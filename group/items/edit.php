...
if ($loggedIn == true) :
    ...
    <div class="front-page-form" >
        <form class="login" >
            <div class="field">
                <input type="text" class="form-control" id="item_name" name="item_name" maxlength="10" <?php echo 'Value=' . '"' . $item_name . '"'; ?>/>
            </div>
            ...
            <div class="field">
                <input type="checkbox" id="personal" name="personal" <?php
                if ($personal == 1) {
                    echo 'checked';
                }
                ?>/> Personal item? (Only you can see this)
            </div>
        ...
        $(document).ready(function () {
            ...
            $('input#submit').on('click', function () {
                ...
                if (itemName != '' || itemDetails != '') {
                    ...
                    $.post('process_edit.php', {item_name: itemName, item_details: itemDetails, priority: priority, due_date: due, file: file, list_id: listId, list_item_id: listItemId, personal: personal}, function (data) {
                        alert(data);
                        $('<form action="/listr/group/items/index.php" method="POST">' +
                '<input type="hidden" name="var" value="' + <?php echo $_POST['list_id']; ?> + '">' +
                '<input type="hidden" name="group_id" value="' + <?php echo $_POST['group_id']; ?> + '">' +
