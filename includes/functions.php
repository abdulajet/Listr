...
include_once ($_SERVER['DOCUMENT_ROOT'] . "/listr/includes/psl-config.php");
...
function login($email, $password, $dbh) {
    ...
    if ($stmt = $dbh->prepare("SELECT user_id, username, password, salt 
        FROM user
       WHERE email = ?
        LIMIT 1")) {
...
function mailUser($to, $subject, $body) {


    $headers = 'From: Listr <NO_REPLY@listr.esy.es>';
